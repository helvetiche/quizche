...
// CONSTANTS
// ============================================================================
const QUESTION_TYPES: { value: QuestionType; label: string; icon: string; description: string }[] = [
  { value: "multiple_choice", label: "Multiple Choice", icon: "radio_button_checked", description: "Select one correct answer from several options" },
  { value: "identification", label: "Identification", icon: "text_fields", description: "Type the exact answer to the question" },
  { value: "true_or_false", label: "True / False", icon: "toggle_on", description: "Determine if the statement is correct" },
  { value: "essay", label: "Essay", icon: "article", description: "Write a detailed response to the prompt" },
  { value: "enumeration", label: "Enumeration", icon: "format_list_numbered", description: "List multiple items that answer the question" },
  { value: "reflection", label: "Reflection", icon: "psychology", description: "Share personal thoughts and insights on topic" },
];
...
export default function ComposerPage() {
  ...
  const handleAIGenerated = (quiz: GeneratedQuizData) => {
    setSettings((prev) => ({
      ...prev,
      title: quiz.title,
      description: quiz.description,
  ...
  const handleAddQuestion = (type: QuestionType = "multiple_choice") => {
    ...
    setTimeout(() => {
      if (paginationRef.current) {
        paginationRef.current.scrollTo({ left: paginationRef.current.scrollWidth, behavior: 'smooth' });
      }
    }, 50);
  };
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === "multiple_choice") {
        ...
        if (validChoices.length < 2) {
          alert(`Question ${i + 1}: Multiple choice questions must have at least 2 choices`);
          setCurrentQuestionIndex(i);
          return false;
        }
        ...
        if (!validChoices.map(c => c.trim()).includes(q.answer.trim())) {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className={`flex flex-col bg-amber-100 border-r-2 border-gray-900 transition-all duration-200 ${sidebarCollapsed ? 'w-16' : 'w-56'}`}>
          ...
          <div className="flex items-center justify-between p-3 border-b-2 border-gray-900">
            {!sidebarCollapsed && <span className="text-gray-900 font-black text-xs uppercase tracking-wider">Tools</span>}
          ...
          <div className="p-2 border-b-2 border-gray-900">
            {!sidebarCollapsed && <p className="text-[10px] font-black text-gray-700 uppercase tracking-wider mb-2 px-2">Add Question</p>}
            <div className="flex flex-col gap-1">
              {QUESTION_TYPES.map((type) => (
                <button key={type.value} onClick={() => handleAddQuestion(type.value)} className={`group flex items-center gap-2 p-2 rounded-xl hover:bg-amber-200 border-2 border-transparent hover:border-gray-900 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`} title={type.label}>
                  ...
                  {!sidebarCollapsed && (
                    <div className="flex flex-col items-start min-w-0 text-left">
                      <span className="text-gray-900 font-bold text-xs">{type.label}</span>
                      <span className="text-gray-500 text-[9px] leading-tight line-clamp-2 text-left">{type.description}</span>
                    </div>
                  )}
                </button>
              ))}
          ...
          {/* Actions */}
          <div className="p-2 border-b-2 border-gray-900">
            {!sidebarCollapsed && <p className="text-[10px] font-black text-gray-700 uppercase tracking-wider mb-2 px-2">Actions</p>}
            <div className="flex flex-col gap-1">
              <button onClick={() => setShowAIModal(true)} className={`group flex items-center gap-2 p-2 rounded-xl hover:bg-amber-200 border-2 border-transparent hover:border-gray-900 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`} title="AI Generate">
                <div className="w-8 h-8 bg-amber-200 rounded-lg flex items-center justify-center flex-shrink-0 border-2 border-gray-900 group-hover:bg-amber-300">
                  <span className="material-icons-outlined text-gray-900 text-sm">auto_awesome</span>
                </div>
                {!sidebarCollapsed && (
                  <div className="flex flex-col items-start min-w-0 text-left">
                    <span className="text-gray-900 font-bold text-xs">AI Generate</span>
                    <span className="text-gray-500 text-[9px] leading-tight line-clamp-2">Generate quiz questions from PDF using AI</span>
                  </div>
                )}
              </button>
              <button onClick={() => setShowSettingsModal(true)} className={`group flex items-center gap-2 p-2 rounded-xl hover:bg-amber-200 border-2 border-transparent hover:border-gray-900 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`} title="Settings">
                ...
                {!sidebarCollapsed && (
                  <div className="flex flex-col items-start min-w-0 text-left">
                    ...
                    <span className="text-gray-500 text-[9px] leading-tight line-clamp-2">Configure quiz title, time limit, and options</span>
                  </div>
                )}
              </button>
              <button onClick={handleDuplicateQuestion} className={`group flex items-center gap-2 p-2 rounded-xl hover:bg-amber-200 border-2 border-transparent hover:border-gray-900 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`} title="Duplicate">
                <div className="w-8 h-8 bg-amber-200 rounded-lg flex items-center justify-center flex-shrink-0 border-2 border-gray-900 group-hover:bg-amber-300">
                  <span className="material-icons-outlined text-gray-900 text-sm">content_copy</span>
                </div>
                {!sidebarCollapsed && (
                  <div className="flex flex-col items-start min-w-0 text-left">
                    <span className="text-gray-900 font-bold text-xs">Duplicate</span>
                    <span className="text-gray-500 text-[9px] leading-tight line-clamp-2">Create a copy of the current question</span>
                  </div>
                )}
              </button>
              {questions.length > 1 && (
                <button onClick={() => handleRemoveQuestion(currentQuestion?.id || '')} className={`group flex items-center gap-2 p-2 rounded-xl hover:bg-red-100 border-2 border-transparent hover:border-red-500 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`} title="Delete">
                  <div className="w-8 h-8 bg-amber-200 rounded-lg flex items-center justify-center flex-shrink-0 border-2 border-gray-900 group-hover:bg-red-200">
                    <span className="material-icons-outlined text-gray-900 text-sm group-hover:text-red-600">delete</span>
                  </div>
                  {!sidebarCollapsed && (
                    <div className="flex flex-col items-start min-w-0 text-left">
                      <span className="text-gray-900 font-bold text-xs group-hover:text-red-600">Delete</span>
                      <span className="text-gray-500 text-[9px] leading-tight line-clamp-2 group-hover:text-red-500">Remove the current question from quiz</span>
                    </div>
                  )}
                </button>
              )}
                  ...
                  <div className="bg-amber-200 px-6 py-4 flex items-center justify-between border-b-2 border-gray-900">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-700 text-sm font-medium">Question {currentQuestionIndex + 1} of {questions.length}</p>
                      </div>
                    </div>
                    <select
                      value={currentQuestion.type}
                      onChange={(e) => handleQuestionChange(currentQuestion.id, "type", e.target.value)}
                  ...
                  <div className="p-6 flex flex-col gap-6 bg-amber-50">
                    ...
                    <div className="flex gap-6">
                      ...
                      <div className="flex-1">
                        <label className="text-sm font-black text-gray-900 mb-2 block">Question <span className="text-red-500">*</span></label>
                        <textarea
                          value={currentQuestion.question}
                          onChange={(e) => handleQuestionChange(currentQuestion.id, "question", e.target.value)}
                      ...
                      <div className="w-64 flex-shrink-0">
                        <label className="text-sm font-black text-gray-900 mb-2 block">Image (Optional)</label>
                        {currentQuestion.imagePreview || currentQuestion.imageUrl ? (
                          <div className="relative h-40 border-2 border-gray-900 rounded-xl overflow-hidden bg-white group">
                            <Image src={currentQuestion.imagePreview || currentQuestion.imageUrl || ""} alt="Question" fill className="object-contain" unoptimized={!!currentQuestion.imagePreview} />
                            <button 
                          ...
                          <label className="flex flex-col items-center justify-center gap-2 h-40 bg-white border-2 border-dashed border-gray-400 rounded-xl cursor-pointer hover:border-gray-900 hover:bg-amber-100 transition-all">
                            <span className="material-icons-outlined text-gray-400 text-4xl">add_photo_alternate</span>
                    ...
                    {currentQuestion.type === "multiple_choice" && (
                      <div>
                        ...
                        {hasDuplicateChoices(currentQuestion.id) && <p className="text-sm text-red-500 font-bold mt-3 flex items-center gap-1"><span className="material-icons-outlined text-sm">warning</span> Duplicate choices</p>}
                        <p className="text-xs text-gray-600 font-medium mt-3">Click the circle to mark the correct answer • Max 6 choices</p>
                      </div>
                    )}
                    ...
                    {currentQuestion.type === "true_or_false" && (
                      <div>
                        <label className="text-sm font-black text-gray-900 mb-3 block">Correct Answer <span className="text-red-500">*</span></label>
                        <div className="flex gap-4">
                          <button onClick={() => handleQuestionChange(currentQuestion.id, "answer", "true")} className={`flex-1 py-4 rounded-xl border-2 font-bold text-lg transition-all ${currentQuestion.answer === "true" ? "bg-green-400 border-gray-900 text-gray-900 shadow-[3px_3px_0px_0px_rgba(17,24,39,1)]" : "bg-white border-gray-300 text-gray-700 hover:border-green-500"}`}>
                            <span className="flex items-center justify-center gap-2"><span className="material-icons-outlined">check</span> True</span>
                          </button>
                          <button onClick={() => handleQuestionChange(currentQuestion.id, "answer", "false")} className={`flex-1 py-4 rounded-xl border-2 font-bold text-lg transition-all ${currentQuestion.answer === "false" ? "bg-red-400 border-gray-900 text-gray-900 shadow-[3px_3px_0px_0px_rgba(17,24,39,1)]" : "bg-white border-gray-300 text-gray-700 hover:border-red-500"}`}>
                    ...
                    {(currentQuestion.type === "identification" || currentQuestion.type === "enumeration" || currentQuestion.type === "essay" || currentQuestion.type === "reflection") && (
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => handleQuestionChange(currentQuestion.id, "answer", e.target.value)}
        ...
        <Modal isOpen={showSettingsModal} onClose={() => setShowSettingsModal(false)} className="w-full max-w-2xl max-h-[90vh]">
          <div className="bg-amber-50 border-3 border-gray-900 rounded-2xl shadow-[8px_8px_0px_0px_rgba(17,24,39,1)] w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b-2 border-gray-900 bg-amber-200">
              ...
              <button onClick={() => setShowSettingsModal(false)} className="w-10 h-10 bg-red-400 rounded-xl flex items-center justify-center border-2 border-gray-900 hover:bg-red-500 transition-colors shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]">
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4">
                <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider">Basic Information</h4>
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-bold text-gray-700">Title <span className="text-red-500">*</span></label>
                  <input type="text" value={settings.title} onChange={(e) => handleSettingChange("title", e.target.value)} placeholder="Enter quiz title..." className="px-4 py-3 bg-white border-2 border-gray-900 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-amber-400 shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-bold text-gray-700">Description</label>
                  <textarea value={settings.description} onChange={(e) => handleSettingChange("description", e.target.value)} placeholder="Enter description..." rows={3} className="px-4 py-3 bg-white border-2 border-gray-900 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-amber-400 resize-none shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]" />
                </div>
              </div>

              {/* Time Settings */}
              <div className="flex flex-col gap-4">
                <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider">Time & Attempts</h4>
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="number" min="0" value={settings.duration || ""} onChange={(e) => handleSettingChange("duration", e.target.value ? parseInt(e.target.value) : null)} placeholder="Custom duration..." className="px-4 py-3 bg-white border-2 border-gray-900 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-amber-400 shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-bold text-gray-700">Max Attempts</label>
                    <input type="number" min="1" value={settings.maxAttempts} onChange={(e) => handleSettingChange("maxAttempts", parseInt(e.target.value) || 1)} className="px-4 py-3 bg-white border-2 border-gray-900 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-amber-400 shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]" />
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {[
                        ...
                        const isSelected = settings.deadline && settings.deadline.slice(0, 10) === presetValue.slice(0, 10);
                        return (
                          <button
                            key={preset.days}
                            onClick={() => handleSettingChange("deadline", presetValue)}
                            className={`px-4 py-2 rounded-full border-2 font-bold text-sm transition-all ${
                              isSelected
                                ? "bg-amber-200 border-gray-900 text-gray-900 shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]"
                                : "bg-white border-gray-300 text-gray-700 hover:border-gray-900"
                    ...
                    <input type="datetime-local" value={settings.deadline} onChange={(e) => handleSettingChange("deadline", e.target.value)} className="px-4 py-3 bg-white border-2 border-gray-900 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-amber-400 shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]" />
                  </div>
                </div>
              </div>
              ...
              {/* Quiz Options */}
              <div className="flex flex-col gap-4">
                <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider">Quiz Options</h4>
                <div className="grid grid-cols-2 gap-3">
                  <ToggleButton label="Shuffle Questions" checked={settings.shuffleQuestions} onChange={(v) => handleSettingChange("shuffleQuestions", v)} />
                  <ToggleButton label="Shuffle Choices" checked={settings.shuffleChoices} onChange={(v) => handleSettingChange("shuffleChoices", v)} />
                  <ToggleButton label="Show Results" checked={settings.showResults} onChange={(v) => handleSettingChange("showResults", v)} />
                  <ToggleButton label="Allow Retake" checked={settings.allowRetake} onChange={(v) => handleSettingChange("allowRetake", v)} />
                </div>
              </div>

              {/* Anti-Cheat */}
              <div className="flex flex-col gap-4">
                <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider">Anti-Cheat</h4>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-2">
                    <ToggleButton label="Prevent Tab Switch" checked={settings.preventTabSwitch} onChange={(v) => handleSettingChange("preventTabSwitch", v)} />
                    {settings.preventTabSwitch && (
                      <div className="flex items-center gap-2 pl-2">
                        <label className="text-xs font-medium text-gray-600">Max violations:</label>
                        <input 
                          type="number" 
                          min="1" 
                          max="10" 
                          value={settings.maxTabSwitches} 
                          onChange={(e) => handleSettingChange("maxTabSwitches", Math.max(1, Math.min(10, parseInt(e.target.value) || 3)))} 
                  ...
                  <ToggleButton label="Prevent Copy/Paste" checked={settings.preventCopyPaste} onChange={(v) => handleSettingChange("preventCopyPaste", v)} />
                  <ToggleButton label="Fullscreen Mode" checked={settings.fullscreenMode} onChange={(v) => handleSettingChange("fullscreenMode", v)} />
                  <ToggleButton label="Disable Right Click" checked={settings.disableRightClick} onChange={(v) => handleSettingChange("disableRightClick", v)} />
                </div>
              </div>

              {/* Instructions */}
              <div className="bg-amber-100 border-2 border-amber-400 rounded-xl p-4">
                <p className="text-gray-900 font-bold text-sm mb-2">How to Configure Your Quiz</p>
                <p className="text-gray-700 text-xs leading-relaxed">Set a clear title and description so students know what to expect. Choose an appropriate duration based on the number and complexity of questions. Use deadline presets for quick scheduling or set a custom date. Enable anti-cheat options for high-stakes assessments to maintain academic integrity during the quiz.</p>
              </div>
            </div>

            {/* Modal Footer */}
            <div className="flex justify-end gap-3 px-6 py-4 border-t-2 border-gray-900 bg-amber-100">
              <button onClick={() => setShowSettingsModal(false)} className="px-6 py-3 bg-amber-200 text-gray-900 font-bold rounded-xl border-2 border-gray-900 hover:bg-amber-300 transition-colors">Cancel</button>
              <button onClick={() => setShowSettingsModal(false)} className="px-6 py-3 bg-amber-400 text-gray-900 font-bold rounded-xl border-2 border-gray-900 shadow-[3px_3px_0px_0px_rgba(17,24,39,1)] hover:shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all flex items-center gap-2">
...
function ToggleButton({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)} className={`flex items-center justify-between p-3 rounded-xl border-2 transition-all ${checked ? "bg-amber-200 text-gray-900 border-gray-900 shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]" : "bg-white text-gray-900 border-gray-300 hover:border-gray-900"}`}>
      <span className="font-bold text-sm">{label}</span>
      <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center ${checked ? "bg-amber-400 border-gray-900" : "bg-white border-gray-400"}`}>
        {checked && <span className="material-icons-outlined text-gray-900 text-sm">check</span>}
      </div>
    </button>
  );
}
