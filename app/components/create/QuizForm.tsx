...
const EXPLANATION_SUPPORTED_TYPES: QuestionType[] = [
  "multiple_choice",
  "identification",
  "true_or_false",
];
...
const QuizForm = ({
  ...
  const [loadingQuiz, setLoadingQuiz] = useState(
    (quizId !== undefined && quizId !== null) ||
      (initialDraftId !== undefined && initialDraftId !== null)
  );
  ...
  const [questions, setQuestions] = useState<Question[]>(
    initialData?.questions
      ? initialData.questions.map((q, index) => ({
          ...
          choiceExplanations:
            q.type === "multiple_choice"
              ? q.choiceExplanations !== undefined &&
                    ...
                    <label className="flex flex-col items-center justify-center gap-2 p-6 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-gray-900 hover:bg-gray-100 transition-all">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) handleImageSelect(currentQuestion.id, file);
                          e.target.value = "";
                        }}
                        className="hidden"
                      />
