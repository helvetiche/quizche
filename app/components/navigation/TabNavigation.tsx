...
export default function TabNavigation({ userRole, badges = {}, onAIClick }: TabNavigationProps) {
      ...
      <div className="flex items-center gap-3">
        ...
        {userRole === "teacher" && (
          <div className="relative">
            ...
            >
              <span className="material-icons-outlined text-gray-900 text-3xl">add</span>
            </button>
            
            {/* Quiz Limit Popover */}
            <div className={`absolute bottom-full mb-3 left-1/2 -translate-x-1/2 bg-amber-100 border-2 border-gray-800 rounded-xl p-3 shadow-[4px_4px_0px_0px_rgba(31,41,55,1)] whitespace-nowrap transition-all duration-200 ${showQuizLimit ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
              <div className="flex items-center gap-2 mb-2">
                <span className="material-icons-outlined text-gray-900 text-sm">assignment</span>
                <span className="font-bold text-gray-900 text-sm">Quiz Limit</span>
              </div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-700">{quizzesCreated}/{quizLimit} created</span>
              </div>
              ...
              <p className="text-xs text-gray-500 mt-1">{quizRemaining} remaining</p>
              {/* Arrow */}
        ...
        {userRole === "teacher" && (
          <div className="relative">
            ...
            >
              <span className="material-icons-outlined text-gray-900 text-3xl">auto_awesome</span>
            </button>
            
            {/* Credits Popover */}
            <div className={`absolute bottom-full mb-3 left-1/2 -translate-x-1/2 bg-amber-100 border-2 border-gray-800 rounded-xl p-3 shadow-[4px_4px_0px_0px_rgba(31,41,55,1)] whitespace-nowrap transition-all duration-200 ${showAICredits ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="material-icons-outlined text-gray-900 text-sm">auto_awesome</span>
                  <span className="font-bold text-gray-900 text-sm">AI Credits</span>
                </div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-medium text-gray-700">{aiCreditsRemaining}/{aiCreditsTotal} remaining</span>
                </div>
                <div className="w-32 h-2 bg-gray-200 rounded-full border border-gray-800 overflow-hidden">
                  <div 
